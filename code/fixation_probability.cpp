...
double delta  = 1.0 ;     // Nonlinearity exponent for migration
double fStar = 0.0 ; // Alee Effect (UNUSED) 

int nHetPoints = 10000 ; 
int tHetInterval = 100 ; // Not maximum time so set arbitrary interval for recording 
int widthCounter = 0 ;  // 
// ~~~~~~~~~~~~~~~~ //
...
void Fixation_initialize(int x)
{
    ...
    ifstream finput("profile_input.txt");
    ...
    while (!finput.eof())
    {
        ...
        if (value_x != "")
        {
            if ( stoi(value_x) >= 0 and stoi(value_x) < nDeme )
            {
                if ( stoi(value_x) != x )
                {
                    dA[stoi(value_x)][0] = long(Kap*stod(value_rho)) ;
                    dA[stoi(value_x)][1] = 0 ;
                ...
                {
                    dA[stoi(value_x)][0] = 0 ;
                    dA[stoi(value_x)][1] = long(Kap*stod(value_rho)) ;
...
{
    return m0 + m1*pow( sp_frac, delta) ;
}
// ------------------------------------------------------------------- //
...
// ------------------------------------------------------------------- //
int check_fixation(long arr[][nSp]) 
// Records the result of the fixation experiment 
{
    ...
    if (species_number == -occ_demes)
        result = -1 ;
    else if (species_number == occ_demes)
        result = 1 ;
    else
        result = 0 ;
...
int main(int argc, char * argv[] ) 
{
  ...
  int c, run = 0 ; 
  while ( (c = getopt (argc, argv, "r:f:m:p:g:K:n:w:x:")) != -1 ) // Params //
  ...
  string fixName = "fixation_N" + str_Kap.str() + "_gf" + str_gR.str() + "_m0" + str_m0.str() + "_m1" + str_m1.str() + termination ; 

  ofstream fpop, fhet, fprof, fparam, ffix ; 
