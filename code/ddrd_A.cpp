...
// ------------------------------------------------------------------- //
void calcNGen( double migrationFactor, double pushedBoundary, double fullyPushedBoundary )
{
  ...
  if ( migrationFactor*A + B < pushedBoundary )
  {
      // Approximate phase boundary by straight line 
      nGen = max( int( 20*tBSC ), 50000 ) ;
  }
  else if ( migrationFactor*A + B >= pushedBoundary and migrationFactor*A + B < fullyPushedBoundary )
  {
...
{
    if (migrationModel == "quadratic")
      return m0*(1.0 + A*pow(sp_frac, 2)) ;
    else if (migrationModel == "sqrt")
      return m0*(1.0 + A*sqrt(sp_frac)) ;
    else
...
void runFixedTime()
  {
  if (migrationModel == "sqrt")
    calcNGen(0.25, 2.0, 4.0) ; // Calculate nGen 
  else if (migrationModel == "quadratic")
    calcNGen(1.0, 2.0, 4.0) ; // Calculate nGen 
  else
    calcNGen(2.0, 2.0, 4.0) ; // Calculate nGen 
  Initialize();

  cout << "Starting fixed time simulations w/ nGen = " << nGen << endl ;
...
int main(int argc, char * argv[] ) 
  ...
  while ( ( c = getopt ( argc, argv, "n:r:B:N:m:A:d:w:x:b:M:" ) ) != -1 ) // Params //
    {
  ...
  str_run << runNumber ; 
  str_nDeme << nDeme ; 
  string termination = "_" + migrationModel + ".txt" ;
  // ~~~~~~~~~~~~~~~~~~~~~//
