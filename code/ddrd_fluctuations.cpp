...
int nHetPoints = 10000 ; 
int nFracPoints = 100 ; 
int tHetInterval = 5 ; 
int tFracInterval = 100 ; 
int widthCounter = 0 ;  // 
// ~~~~~~~~~~~~~~~~ //
...
//
//
void simulationLoop(int t, bool saveProfiles, int runLabel)
  {
  Mig_del_1();
  Growth(); 
  ...
  // Hack to output all profiles
  ofstream fProfiles ;
  if(saveProfiles == true)
    {
    ostringstream profilesFName ;
    profilesFName << "profile_N" << populationSize << "_r" << r0 << "_m" << m0 << "_A" << fixed << setprecision(1) << A << "_B" << B << "_run" << runLabel << ".txt" ; 
    fProfiles.open(profilesFName.str(), fstream::app) ;
    }
  ...
  if ((t % tFracInterval == 0) and (saveProfiles == true))
    {
    // print profile to file
    fProfiles << t << ',' ;
    for( int i = 0; i < nDeme - 1; i++ )
      fProfiles << dA[i][0] << ',' << dA[i][1] << ',';
    fProfiles << dA[nDeme - 1][0] << ',' << dA[nDeme - 1][1] << endl;
    }
  }


void runFixedTime(bool saveProfiles, int runLabel)
  {
  Initialize();

  for (unsigned int t = 0; t <= nGen ; t++ )
    {   
    simulationLoop(t, saveProfiles, runLabel) ;
    }
  }
...
int main(int argc, char * argv[] ) 
  ...
  while ( ( c = getopt ( argc, argv, "n:r:B:N:m:A:d:w:x:b:T:" ) ) != -1 ) // Params //
    {
  ...
  // Assign output intervals
  tHetInterval = nGen / nHetPoints ;
  tFracInterval = nGen / nFracPoints ;
  ...
  if( fix == true )
    ...
    while( checkFixation( dA ) == -1 )
      {   
      simulationLoop(t, false, runNumber) ;
      t++ ;
      }
    ...
    {
    cout << "Starting fixed time simulations..." << endl ;
    cout << "Simulation time: " << nGen << endl ;
    cout << "Heterozygosity output interval: " << tHetInterval << endl ;
    cout << "Fraction distribution output interval: " << tFracInterval << endl ;

    runFixedTime(true, runNumber) ;
    }
  clock_t c_fin = clock() ; 
  // ~~~~~~~~~~~~~~~~~~~~~~~ //
